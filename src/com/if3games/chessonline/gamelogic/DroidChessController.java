...
import com.if3games.chessonline.GUIInterface;
import com.if3games.chessonline.GameMode;
import com.if3games.chessonline.PGNOptions;
import com.if3games.chessonline.Util;
import com.if3games.chessonline.book.BookOptions;
import com.if3games.chessonline.engine.DroidComputerPlayer;
import com.if3games.chessonline.engine.DroidComputerPlayer.SearchRequest;
import com.if3games.chessonline.engine.DroidComputerPlayer.SearchType;
import com.if3games.chessonline.gamelogic.Game.GameState;
...
public class DroidChessController {
    ...
    public final synchronized void makeHumanMoveGMS(Move m, boolean network) {
        if (network) {
            Position oldPos = new Position(game.currPos());
            if (doMove(m)) {
                if (m.equals(ponderMove) && !gameMode.analysisMode() &&
    ...
    /** Accept draw gms game for current player. */
    public final synchronized void acceptDrawGmsGame() {
        if (game.getGameState() == GameState.ALIVE) {
            game.processString("gms_accept");
            updateGUI();
        }
    }
    ...
    private final class SearchListener implements com.if3games.chessonline.gamelogic.SearchListener {
